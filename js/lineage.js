  ...
  function createLineage() {
    ...
    function buildTree() {
      ...
      function unitOf(p) {
        ...
        /* เรียงพี่ไปน้องด้วย "เดือนเกิด" ที่ตายตัว ห้ามใช้อายุปี — พี่น้องวัย
           ไล่เลี่ยกันจะอายุเท่ากันเป็นช่วงๆ ของปี พอใครถึงวันเกิดก่อนลำดับจะ
           พลิกไปมา การ์ดสลับตำแหน่งกันแล้วเส้นปะดูเหมือนสลับเป้า (เจอกับ
           บุตรลับสองคนชื่อซ้ำกัน) เดือนเกิดคงที่ตลอดชีวิตจึงไม่มีวันพลิก */
        const bornOf = (k) => (k.bornMonth != null ? k.bornMonth : -k.age * 12);
        return {
          person: p,
          spouse: sp,
          children: kids
            .sort((x, y) => bornOf(x) - bornOf(y))
            .map(unitOf)
            .filter(Boolean),
        };
      }
